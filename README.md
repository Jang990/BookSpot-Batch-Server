...
### 🏛️ 도서관 동기화 작업
- **도서관 정보 최신화** (ChunkSize -  200)
    - BeforeStep: 정보나루에서 도서관 Excel 파일 다운로드
    - Reader: 다운로드된 도서관 Excel 파일 읽기 - `close()` 호출 시 읽은 파일 삭제
    - Writer: `Insert ... ON DUPLICATE KEY UPDATE`

<br>

- **도서관 장서 파일 다운로드를 위한 naru_detail 크롤링** (ChunkSize -  200)
...
### 📥 도서관 장서 파일 다운로드 작업
약 1500개의 파일. 17.5GB의 전체 파일 크기

- **도서관 장서 파일 다운로드** (ChunkSize - 10)
    - Reader: 도서관 테이블에서 크롤링에 필요한 정보 가져오기
    - Processor: 가장 최근 장서 csv 파일 경로를 크롤링
    - Writer: 도서관 장서 csv 파일 다운로드
...
- **DB에 없는 신규 책 정보 저장** (파티셔닝 Master Step - 멀티 스레딩)
    - beforeStep: Book 테이블에서 ISBN13을 읽어와서 `IsbnSet` 초기화
    - Slave Step (ChunkSize - 1,300)
...
약 280만 권의 책 데이터의 대출 수를 집계
```

- **인메모리 대출 수 집계 + 파일 생성**  (파티셔닝 Master Step - 멀티스레딩)
    ...
    - Slave Step - `allowStartIfComplete(true)` (ChunkSize - 1,000)
        - Reader: 도서관 소장 도서 csv 파일 읽기
        - Processor
            - 유효하지 않은 ISBN13 필터링
            - `{ISBN13-대출 수}`객체로 변환
...
- **집계된 대출 수 정보 반영** (ChunkSize - 3,000)
    - Reader: 대출 수 종합 파일읽기
...
도서관 장서 파일에 새롭게 등장한 책 Insert
도서관 장서 파일에 없어진 책 Delete
```

- **도서관 장서 CSV 파일 정제 (책제목,발행년도,저자,... => 책ID,도서관ID)** (파티셔닝 Master Step - 멀티스레딩)
  - beforeStep: `Map<ISBN13,BookDbId>` 초기화
  - SlaveStep (ChunkSize - 800)
...
- **중복된 {책ID, 도서관ID} 제거 ( `{1,1},{2,1},{1,1} => {1,1},{2,1}` )** (파티셔닝 Master Step - 멀티스레딩)
  - SlaveStep (ChunkSize - 1,500)
...
- **DB에 존재하지 않는 책 Insert** (파티셔닝 - 멀티스레딩) (ChunkSize - 15_000) 
...
- **DB에 있지만 파일에 존재하지 않는 책 식별** (파티셔닝 - 멀티스레딩)
...
- **책 Delete** (파티셔닝 - 멀티스레딩) (ChunkSize - 5,000)
...
- **OpenSearch 신규 책 인덱스 생성**
  - (25년 6월 기준) books-2025-06 인덱스 생성
- **책 정보 OpenSearch 적재** (ChunkSize - 150)
  - Reader: 책 테이블 정보 읽기
